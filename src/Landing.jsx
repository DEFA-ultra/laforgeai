    ...
    <div style={{ minHeight: "100vh", background: "#07050f", fontFamily: "'DM Sans',sans-serif", color: "#cffafe", overflowX: "hidden" }}>
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .gen-card:hover { border-color: ${C} !important; transform: translateY(-4px); box-shadow: 0 20px 60px rgba(6,182,212,0.15) !important; }
        .gen-card { transition: all 0.25s ease; }
      ...
      <section style={{ padding: "5rem 1.5rem 4rem", textAlign: "center", maxWidth: 800, margin: "0 auto" }}>
        <div className="fade-up" style={{ display: "inline-block", background: `${C}18`, border: `1px solid ${C}44`, borderRadius: 20, padding: "0.35rem 1rem", fontSize: "0.75rem", color: C, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "1.5rem" }}>
          🔨 Powerlifting & Musculation — Programme IA personnalisé
        </div>
      ...
      <section style={{ padding: "4rem 1.5rem", maxWidth: 900, margin: "0 auto" }}>
        ...
        <p style={{ textAlign: "center", color: "#1a4050", marginBottom: "3rem", fontSize: "0.9rem" }}>Deux programmes spécialisés pour deux disciplines</p>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1.5rem" }}>

          {/* POWERLIFTING */}
          <div className="gen-card" style={{ background: "#080612", border: `1px solid #1a2535`, borderRadius: 16, padding: "2rem", display: "flex", flexDirection: "column" }}>
            <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>🏋️</div>
            <div style={{ fontFamily: "'Bebas Neue',cursive", fontSize: "1.8rem", color: "#fff", letterSpacing: "0.05em", marginBottom: "0.5rem" }}>POWERLIFTING</div>
            <div style={{ color: C, fontSize: "0.8rem", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "1.2rem" }}>Squat • Bench Press • Deadlift</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "1.5rem", flex: 1 }}>
              {["Top Set + Back-off calculés depuis tes 1RM", "Fréquence par mouvement personnalisée", "Variantes SBD et accessoires", "Progression S1 à S16 avec charges exactes", "Nutrition adaptée au powerlifter"].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                  <span style={{ color: C, fontSize: "0.9rem", flexShrink: 0 }}>✓</span>
                  <span style={{ color: "#7dd3e0", fontSize: "0.85rem", lineHeight: 1.5 }}>{item}</span>
                </div>
              ))}
            </div>
            <a href={STRIPE_LINK} className="cta-btn" style={{ display: "block", background: `linear-gradient(135deg, ${C}, #0891b2)`, color: "#07050f", padding: "0.85rem", borderRadius: 10, fontWeight: 700, fontSize: "1rem", textDecoration: "none", fontFamily: "'Bebas Neue',cursive", letterSpacing: "0.1em", textAlign: "center", boxShadow: `0 4px 24px ${C}44` }}>
              🔨 GÉNÉRER — 19€
            </a>
            <a href="/generator" style={{ display: "block", textAlign: "center", color: "#1a4050", fontSize: "0.75rem", marginTop: "0.6rem", textDecoration: "none" }}>
              Voir le générateur →
            </a>
          </div>
          ...
          <div className="gen-card" style={{ background: "#080612", border: `1px solid #1a2535`, borderRadius: 16, padding: "2rem", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ fontFamily: "'Bebas Neue',cursive", fontSize: "1.8rem", color: "#fff", letterSpacing: "0.05em", marginBottom: "0.5rem" }}>MUSCULATION</div>
            <div style={{ color: C, fontSize: "0.8rem", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "1.2rem" }}>Masse • Sèche • Recomposition</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "1.5rem", flex: 1 }}>
              {["Full Body, PPL, Arnold Split, Powerbuilding", "Calories & macros calculés (Mifflin-St Jeor)", "Adaptation au matériel disponible", "Gestion blessures avec substitutions", "Préférences alimentaires incluses"].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                  <span style={{ color: C, fontSize: "0.9rem", flexShrink: 0 }}>✓</span>
                  <span style={{ color: "#7dd3e0", fontSize: "0.85rem", lineHeight: 1.5 }}>{item}</span>
                </div>
            ...
            <a href="/musculation" style={{ display: "block", textAlign: "center", color: "#1a4050", fontSize: "0.75rem", marginTop: "0.6rem", textDecoration: "none" }}>
              Voir le générateur →
            </a>
          </div>
      ...
      <section style={{ padding: "4rem 1.5rem", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1rem" }}>
          {[
            { icon: "🏋️", title: "Programme complet personnalisé", desc: "Séances détaillées avec charges calculées selon ton profil exact." },
            { icon: "📅", title: "Progression semaine par semaine", desc: "Chaque semaine planifiée avec intensité et volume progressifs." },
            { icon: "🔥", title: "Échauffement spécifique", desc: "Protocole d'activation détaillé avant chaque séance." },
            ...
            { icon: "📄", title: "PDF téléchargeable", desc: "Export immédiat en PDF professionnel. Imprime ou utilise sur mobile." },
          ].map((f, i) => (
      ...
      <section style={{ padding: "4rem 1.5rem", background: "#080612", borderTop: "1px solid #0e1f2a", borderBottom: "1px solid #0e1f2a" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "2rem" }}>
            {[
              { num: "01", title: "Remplis ton profil", desc: "Tes infos, ton objectif, ton niveau et tes jours d'entraînement." },
              { num: "02", title: "L'IA génère", desc: "Programme complet créé en moins de 60 secondes, personnalisé pour toi." },
              { num: "03", title: "Télécharge ton PDF", desc: "Programme prêt à suivre, téléchargeable immédiatement." },
            ].map((s, i) => (
              <div key={i} style={{ textAlign: "center" }}>
                <div style={{ fontFamily: "'Bebas Neue',cursive", fontSize: "3.5rem", lineHeight: 1, background: `linear-gradient(135deg, ${C}, #0891b2)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{s.num}</div>
      ...
      <section style={{ padding: "4rem 1.5rem", maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
        <div style={{ background: "#080612", border: `1px solid ${C}44`, borderRadius: 16, padding: "2.5rem" }}>
          ...
          <blockquote style={{ fontFamily: "'Bebas Neue',cursive", fontSize: "1.6rem", color: "#fff", lineHeight: 1.3, marginBottom: "1.2rem", letterSpacing: "0.02em" }}>
            &ldquo;La Forge AI a été développée par un athlète de powerlifting qui connaît les exigences du sport — pas par des développeurs qui n&apos;ont jamais touché une barre.&rdquo;
          </blockquote>
      ...
      <section style={{ padding: "4rem 1.5rem", background: "#080612", borderTop: "1px solid #0e1f2a" }}>
        <div style={{ maxWidth: 500, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ background: "#07050f", border: `2px solid ${C}`, borderRadius: 16, padding: "2.5rem", marginBottom: "1.5rem" }}>
            <div style={{ fontSize: "0.8rem", color: C, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: "0.5rem" }}>Powerlifting ou Musculation</div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", marginBottom: "0.25rem" }}>
              <span style={{ fontFamily: "'Bebas Neue',cursive", fontSize: "2.2rem", color: "#1a4050", textDecoration: "line-through" }}>29€</span>
              <span style={{ fontFamily: "'Bebas Neue',cursive", fontSize: "4rem", color: "#fff", lineHeight: 1 }}>19€</span>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.7rem", marginBottom: "2rem", textAlign: "left" }}>
              {["Programme complet personnalisé", "Charges calculées selon ton profil", "Nutrition & récupération incluses", "Progression semaine par semaine", "Export PDF immédiat"].map((item, i) => (
      ...
      <section style={{ padding: "3rem 1.5rem", maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: "0.8rem", justifyContent: "center", flexWrap: "wrap" }}>
          {["🔗 Street Lifting", "⚡ Cross Training", "🏆 Compétition avancée", "📊 Suivi progression"].map((s, i) => (
